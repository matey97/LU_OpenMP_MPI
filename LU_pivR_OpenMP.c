  ...
  while ( ( info == 0 ) && ( time < tmin )) {
    ...
    #pragma omp parallel private(k)
    {
      for (k = 1; k <= dim; k++) {
        ...
        #pragma omp single //Necesaria la barrera de single
        {
          piv = Aref(k,iPiv);
        }
        ...
        #pragma omp master //Aprovechamos el barrier del siguiente for
        {
          bref(k) /= piv;
        }
      ...
      for (k = dim; k > 0; k--) {
        #pragma omp for //La k ya es privada por la region paralela
        for (i = 1; i <= k - 1; i++) {
